...
//     // Scale float and convert to int (e.g., 4 digits after decimal)
//     int32_t int_part = (int32_t)x;
//     int32_t frac_part = (int32_t)((x - (float)int_part) * 10000.0f); // 4 decimal places
    
//     print_int(int_part);
//     uart_write(".",1);
//     print_int(frac_part); // Could zero-pad this if needed
// }
void print_float(float x) {
    if ((int32_t)x < 0) {
        uart_write("-", 1);
