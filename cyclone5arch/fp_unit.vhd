...
signal mult,add,conved_float_toint,conved_int_tofloat,operation,MIN,MAX, selected_B:std_logic_vector(operand_width-1 downto 0);
constant zero:std_logic_vector(operand_width-1 downto 0):=(others=>'0');
constant NAN:std_logic_vector(operand_width-1 downto 0):=(operand_width-1 downto mantissa_width-2 => '1', others => '0');--x"7fc00000"(others=>'0');
constant INF : std_logic_vector(operand_width-1 downto 0) := (operand_width-1 downto mantissa_width => '1',  others       => '0');
constant all_ones:std_logic_vector(exponent_width-1 downto 0):=(others=>'1');
signal conv_int_tofloat_done,mult_done,add_done,go_mul,go_add,div,infinity1,infinity2,zero1,zero2,equal_expo_mantis,equal_ops,equal_literally,control_done,go_conv_int_tofloat,conv_float_toint_done,go_conv_float_toint:std_logic;
signal output_control:std_logic_vector(3 downto 0);
signal a_eq_b,a_lt_b,a_gt_b: std_logic;
begin 

check_correct_length(operand_width,mantissa_width,exponent_width);

equal_ops<='1' when equal_expo_mantis='1' and A(operand_width-1)/=B(operand_width-1) else '0';

equal_literally<= '1' when equal_expo_mantis='1'and A(operand_width-1)=B(operand_width-1) else '0';

equal_expo_mantis<='1' when A(operand_width-2 downto 0)=B(operand_width-2 downto 0) else '0';
...
selected_B<= ((not B(operand_width-1))&B(operand_width-2 downto 0)) when op="1001" else B;

ADDITION:fp_add_sub generic map(operand_width,mantissa_width,exponent_width) port map(clk,rst,go_add,A,selected_B,add_done,add);

CONTROL:fp_unit_control port map(clk,op,infinity1,infinity2,zero1,zero2,equal_ops,equal_literally,output_control,div,go_add,go_mul,go_conv_int_tofloat,go_conv_float_toint,control_done);

operation<=mult when output_control="0000" else
		add when output_control="0001" else 
		zero when output_control="0010"else
		(A(operand_width-1)xor B(operand_width-1))&INF(operand_width-2 downto 0) when output_control="0011" else --inf "111"&x"F800000"
		...
		zero(operand_width-2 downto 0)&a_eq_b when output_control="1011" else
		zero(operand_width-2 downto 0)&a_lt_b when output_control="1100" else
		zero(operand_width-2 downto 0)&(a_eq_b or a_lt_b) when output_control="1101"
		else (others=>'0');
...
COMPARATOR:fp_compare generic map(operand_width,mantissa_width,exponent_width) port map(A,B,a_eq_b,a_lt_b,a_gt_b,MAX,MIN);
